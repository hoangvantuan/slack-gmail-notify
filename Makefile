...
stg:
	kill `cat ./stg.pid` || true
	go get -u github.com/mdshun/slack-gmail-notify
	SLGMAIL_ENV=stg slack-gmail-notify > /dev/null 2>&1 & echo $$! > ./stg.pid
...
tunnel-server:
	kill `cat ./tunnel-server.pid` || true
	tunneld -httpAddr :5000 -httpsAddr :5001 -tlsCrt .tunnel/server.crt -tlsKey .tunnel/server.key > /dev/null 2>&1 & echo $$! > ./tunnel-server.pid
.PHONY: caddy
caddy:
	kill `cat ./caddy.pid` || true
	caddy > /dev/null 2>&1 & echo $$! > ./caddy.pid